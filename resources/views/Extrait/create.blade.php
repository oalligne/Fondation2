...
@section('contenu')
	<div class="col-sm-offset-4 col-sm-4">
		...
		<div class="panel panel-primary">	
			<div class="panel-heading">Création d'un extrait</div>
			<div class="panel-body"> 
				<div class="col-sm-12">
					{!! Form::open(['route' => 'extrait.store', 'class' => 'form-horizontal panel']) !!}	
					...
					<div class="form-group {{ $errors->has('morceaux') ? 'has-error' : '' }}">
						<select class="form-control" id="morceaux" name="morceaux">
						    @foreach ($morceaux as $morceau)
						        <option value="{{ $morceau->id }}">{{ $morceau->nom }}</option>
						    @endforeach
						</select>
					</div>
