...
@section('contenu')
	<div class="col-sm-offset-4 col-sm-4">
		...
		<div class="panel panel-primary">	
			<div class="panel-heading">Création d'un quiz</div>
			<div class="panel-body"> 
				<div class="col-sm-12">
					{!! Form::open(['route' => 'quiz.store', 'class' => 'form-horizontal panel']) !!}	
					...
					<div class="form-group {{ $errors->has('typesquiz') ? 'has-error' : '' }}">
						<select class="form-control" id="typesquiz" name="typesquiz">
						    @foreach ($typesquiz as $typequiz)
						        <option value="{{ $typequiz->id }}">{{ $typequiz->code }}</option>
						    @endforeach
						</select>
					</div>
